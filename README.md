...
When a chain you built while cooking is done, Sizzle asks whether to keep it as a recipe. Recipes live at the top of the **Prep** screen; the mic starts one by name ("prep the stir fry"). The quick New timer path is untouched.

**The recipe screen** (tap a recipe on the Prep screen) is the one screen in Sizzle that scrolls: how many it serves (a stepper, which scales the amounts), the ingredients, the steps as a graph, and one Prep button. Ingredients come from the instructions: write `Add [Diced chicken]` and "Diced chicken" joins the list, tap it to say how much; the instruction card then reads "Add **300 g diced chicken**", scaled to the servings chosen. The graph shows what waits for what. Tapping a step offers to add a step after it (a branch, when it already has one) or remove it; "+ Next step" on a card adds after *everything* currently at the end, which is how two branches join. When a fork's steps come up together, the branch with the most cooking ahead goes on now and the others wait ("Start in 16:00", like Sync Finish) so the branches land together. Engine: `src/lib/recipe.ts` (steps, runs, layout, fork delays), `src/lib/ingredients.ts` (brackets, scaling, "1½ tbsp"); both pure and tested.
