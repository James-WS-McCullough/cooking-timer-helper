...
- **What first use downloads (once, about 90 MB):** the model (77 MB, from Hugging Face, kept in the browser's cache) and its ONNX runtime (14 MB), hosted by Sizzle at `/listen/` the same way as the voice's. The mic says so before fetching anything, and nothing of it loads until then. While the choice is being judged on real phones, opening the app once with `?mic=moonshine` switches that device to [Moonshine](https://github.com/moonshine-ai/moonshine) base (MIT): four times quicker to answer, a little less accurate, a bigger download; `?mic=whisper` switches back.
...
- **Whisper base.en**, the mic's speech-to-text model (downloaded on first use, not shipped in the repo): MIT, by OpenAI; ONNX conversion by Xenova / the transformers.js project. The alternative behind `?mic=moonshine`, **Moonshine base**: MIT, by Useful Sensors / Moonshine AI.
