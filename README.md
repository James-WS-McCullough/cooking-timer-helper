...
When a chain you built while cooking is done, Sizzle asks whether to keep it as a recipe. Recipes have their own button on the start screen (and a row in Settings while timers run): a list to scroll, and **+ New recipe** to build one from scratch, servings and ingredients first. The mic starts one by name ("prep the stir fry"). Neither timer path is touched.
