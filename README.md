...
The list-plus button on a card (beside the bell) adds the next step of a recipe: **an instruction** ("Move the veg to a bowl and cover") or **a timer** (the next thing to cook). Once a chain has two steps, that button shows the chain as a graph instead: tap a step to change it, add a step after it or remove it, add to the end, or keep the lot as a recipe there and then. Steps go on the end of that card's chain, and nothing else appears until it's their turn: when a timer finishes and you press Done, the next step takes its place on screen. An instruction is a card with the words and one Done; a timer step arrives ready, and starts when you press Start, like any prepped timer. Sizzle reads instructions aloud if her voice is on. A chain isn't Sync Finish material (the recipe decides when each thing goes on), and ✕ on any of its cards removes the whole chain.

When a chain you built while cooking is done, Sizzle asks whether to keep it as a recipe. Recipes have their own button on the start screen (and a row in Settings while timers run): a list to scroll, and **+ New recipe** to build one from scratch, servings and ingredients first. **Import a recipe** (on the Recipes page) copies a prompt to give to an AI along with any recipe (text, a link, a photo); paste its reply into the box and it's a recipe, with ingredients, servings and steps. The prompt documents a plain JSON shape (`src/lib/recipeImport.ts`: steps in order, `after` only where a step isn't simply next, minutes not milliseconds); parsing never throws and says what's wrong and where, and tolerates code fences and chatter around the JSON. The mic starts one by name ("prep the stir fry"). Neither timer path is touched.

**The recipe pages** (tap a recipe in the list) replace the timer list rather than floating over it, since a recipe is a document, not a decision: one page for how many it serves (a stepper, which scales the amounts) and the ingredients, with one Prep button; and a Steps page with the graph and room to work on it. Ingredients come from the list (+ Add an ingredient) or from the instructions: write `Add [Diced chicken]` and "Diced chicken" joins the list, tap it to say how much; when writing an instruction the recipe's ingredients are chips, one tap to name one; the instruction card then reads "Add **300 g diced chicken**", scaled to the servings chosen. The graph shows what waits for what. Tapping a step offers to add a step after it (a branch, when it already has one) or remove it; "+ Next step" on a card adds after *everything* currently at the end, which is how two branches join. When a fork's steps come up together they are all simply ready to start, each when it suits the cook (the engine can line branches up to finish together, `forkDelays`, but the app doesn't). Engine: `src/lib/recipe.ts` (steps, runs, layout, fork delays), `src/lib/ingredients.ts` (brackets, scaling, "1½ tbsp"); both pure and tested.

## Watching while you cook

Settings → **Watch while you cook** (a corner button on the start screen) takes a YouTube or Twitch link and plays it inside Sizzle: above the cards on a phone, staying put while they scroll; beside them on a wide screen. Nothing is drawn over the picture. Because the video is Sizzle's own audio, the alarms play over it whichever way the alarms setting is set. `src/lib/watch.ts` (pure, tested) reads the links people paste (youtu.be, shorts, live, channels; twitch.tv channels and videos) and builds the embed, through YouTube's no-cookie player; Twitch is told the embedding site, as it requires. The last link is remembered and offered back; nothing autoplays. Videos whose owners have disabled embedding show YouTube's own "unavailable" notice, and nothing can be done about those.
