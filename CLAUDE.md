...
- Mobile audio: only after a user gesture; iOS's `navigator.audioSession.type` is `'transient'` (mixes with the cook's music; `'playback'` stopped their music every time the app came to the front) except for the seconds an alarm sounds, when `'playback'` is borrowed because `transient` is silenced by the ring/silent switch (verified on the owner's phone), and `'play-and-record'` while the mic records; the silent switch itself is unreadable from a web page; a home-screen web app cannot sound while locked or backgrounded, hence the wake lock.
