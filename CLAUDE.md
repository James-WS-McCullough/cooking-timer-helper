...
- `src/store.ts` is the only stateful module: one reactive `state`, a 250ms `tick()` that advances timers, picks the sound and manages the wake lock (it publishes `state.now` only when the second changes, so cards re-render once a second; an action that changed a timer calls `tick(true)`), and thin actions that call the engine + play a sound. Persistence goes through `src/lib/storage.ts`.
...
- Recipes (`src/lib/recipe.ts`, pure, tested): steps (timer or instruction) with `after` edges; a run is a recipe in progress. The store materialises an active timer step as a prepped `Timer` with a `step` ref and an instruction as a `Note` (`state.notes`); Done on either calls `completeStep` and brings up what follows *in the finished card's place* (`arrivals` in the store, spliced into `shownOrder` in `App.vue`). "+ Next step" is the list-plus button beside the bell: for a lone timer it opens `NextStepSheet.vue` (a timer step reuses `NewTimerSheet` with `after`) and appends to the chain's end; once the chain has 2+ steps it opens the live graph (`StepsPage` with a run), where a node offers Change / Add after / Remove (upcoming steps only for the first and last) and "Keep as a recipe" saves early. A step timer never auto-starts (it's prepped: Start when ready) and is excluded from Sync Finish; ✕ on a step removes the whole run. A finished unnamed chain of 2+ steps offers "Keep that as a recipe?"; recipes live behind a Recipes button on the start screen and a Settings row (`RecipesPage`: list, New recipe, Import), never on either timer path; `ImportPage` + `src/lib/recipeImport.ts` (pure, tested: `IMPORT_PROMPT` and `parseRecipeJson`, whose format the prompt's own example must always parse) turn an AI's JSON reply into a recipe; the mic starts one by name. Recipe pages (`src/components/pages/`: `RecipesPage`, `RecipePage` for servings + ingredients, `StepsPage` for the graph; `PageShell` gives the back arrow and focus) replace the timer list via `page` in `App.vue`, and the step sheets open over them; the servings chosen live in App (`servesChoice`) so a trip between pages keeps them. Ingredients (`src/lib/ingredients.ts`: `[Name]` in an instruction creates one; amounts scale from `recipe.serves` to `run.serves`), `RecipeGraph.vue` (rows from `layout()`, SVG lines measured from the DOM), node tap → add after (`addStep` with a `recipe` target: a fork) or remove. A fork's steps all come up prepped (the owner declined aligning them; `forkDelays` in the engine is unused). `RecipeGraph` measures its lines from layout offsets, not bounding rects, because it first draws mid pop-animation. The card hint lives under the clock, not in the header, which has three buttons.
- Watching (`src/lib/watch.ts` pure link parser + embed URL, `watching.ts` state, `WatchSheet.vue`, `WatchPanel.vue`): the video goes in `.stage` in `App.vue`, sticky above the cards on a phone, a two-column grid at ≥900px; never draw anything over the iframe. E2E stubs the embed hosts with `context().route`. The link input is `type="text"` on purpose: `type="url"` refuses "twitch.tv/x".
- Food data: `src/data/foods.ts` (searchable list), `src/lib/foodIcons.ts` (name → icon), `src/lib/foodSearch.ts` (ranking). Icons are Fluent Emoji via `unplugin-icons`; every icon name used must be imported in `foodIconSet.ts` (it's a type error otherwise), which is a lazy chunk (half the app's code) that `FoodIcon.vue` loads and `App.vue` prefetches on idle.
...
- Mobile audio: only after a user gesture; iOS's `navigator.audioSession.type`: `'transient'` mixes with the cook's music but is silenced by the ring/silent switch; `'playback'` ignores the switch but stops the music and iOS never gives it back (all verified on the owner's phone; suspending the context didn't help either). So it's the cook's choice, `alarmStyle` in `audio.ts` (Settings → "Alarms over music"): `interrupt` (default) takes `'playback'` from 2 s before anything is due (`msUntilNeeded`; switching at the alarm lost its first second to the cross-fade) until nothing waits on the cook, `mix` never does; `'play-and-record'` while the mic records; the silent switch itself is unreadable from a web page; a home-screen web app cannot sound while locked or backgrounded, hence the wake lock.
