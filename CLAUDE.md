...
- Mobile audio: only after a user gesture; iOS's `navigator.audioSession.type` is `'transient'` (mixes with the cook's music; `'playback'` stopped their music every time the app came to the front), but `transient` is silenced by the ring/silent switch (verified on the owner's phone), so `setAlarmMode` takes `'playback'` from 5 s before anything is due (`msUntilNeeded`) until nothing waits on the cook, then hands it back and suspends the context (`resting`, which keeps `soundReady` true) because iOS only resumes the music once a page stops using audio; switching at the alarm itself lost its first second to iOS's cross-fade; `'play-and-record'` while the mic records; the silent switch itself is unreadable from a web page; a home-screen web app cannot sound while locked or backgrounded, hence the wake lock.
