...
const ICONS: Record<FoodIconName, Component> = {
  bacon: Bacon,
  bagel: Bagel,
...
const icon = computed(() => ICONS[props.icon ?? foodIconFor(props.name ?? '')])
</script>

<template>
  <component :is="icon" class="food-icon" aria-hidden="true" />
</template>
